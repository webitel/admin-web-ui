...
var joint = {
    ...
    util: {
        ...
        //  глибокення копіювання всіх властивостей. Якщо властивість повторюється, вона не копіюється
...
/**
 * +++ Пройшовся по всіх методах
 * 
 * Головна модель яка зберігає в собі всі елементи і лінки
 */
joint.dia.Graph = Backbone.Model.extend({
    ...
    initialize: function(attrs, opt) {
        ...
        //  Відслідковує всі івенти на колекції елементів, просто викликає метод з ліби BB, який опрацьовує всі івенти
        //  
     ...
     * +++
     * 
     * Перевіряє чи в елемента є влативість z, якщо ні тоді вирахувати і присвоїти її
     */
    _prepareCell: function(cell) {

        //  якщо елемент створений за допомогою конструктора Backbone.Model і не має властивості "z"
    ...
    //  +++
    //  
    //  Вертає z останнього елемента графа
    //  ??? На що вказує z властивість
    maxZIndex: function() {
     ...
     * +++
     * 
     * Додає новий елемент в graph. Якщо передається масив елементів, тоді всі вони додаються також
     *
     * ??? Вюха для елемента генерується сама
     */
    addCell: function(cell, options) {
        ...
        //  add викликається з ліби BB. Генерить івент add, походу на який відмальовується вюха
        //  метод add викликається із прототипа обєкта
        //  _prepareCell опрацьовує властивість cell.z і додає елемент в масив елементів
    ...
    //  +++
    //
    //  opt.inbound === true || opt.outbound === true || opt.deep === true
    //  Get all inbound and outbound links connected to the cell `model`.
    getConnectedLinks: function(model, opt) {
    ...
    //  +++
    //
    //  Повертає масив елементів, які зєднанні з переданим елементом через лінки
    ...
    //  +++
    //
    //  Вертає масив всіх елементів, межі яких попадають в p { x: ... , y: ... }
    ...
    //  +++
    //  
    //  Find all views in given area r { x: ... , y: ... , width: ... , height: ... }
    ...
    //  +++
    //
    //  Повертає межі рамки для переданих елементів return { x: ... , y: ... , width: ... , height: ... }
    ...
    //  +++
    //
    //  Повертає предка який є спільним для декількох переданих елементів
    getCommonAncestor: function(/* cells */) {
...
//  +++
//  
//  В моделі є властивість attributes, яка має в собі властивість attr, яка містить атрибути для SVG
//
//  Базова модель для лінка і елемента
...
joint.dia.Cell = Backbone.Model.extend({
    ...
    constructor: function(attributes, options) {
        ...
        //  по дефолту в елемента нема своїх атрибутів
        this.attributes = {};

        //  якщо в опціях передана колекція, зберегти її в властивості обєкта
        ...
        //  _.result верне значення властивості defaults для переданого обєкта
        if (defaults = _.result(this, 'defaults')) {
            //<custom code>
            // Replaced the call to _.defaults with _.merge.
            //  мержить дефолтні атрибути із переданими
    ...
    //  +++
    //
    //  Видаляє елемент із графа, всі його внутрішні елементи, всі лінки
    ...
    //  ??? не перевіряв
    //  змінити z-index елемента відносно інших елементів
    ...
    //  +++ Не перевіряв
    //
    //  Вбудувати елемент в даний елемент
    ...
    //  +++ Не перевіряв
    //
    //  Витягнути елемент із даного елемента
    ...
    //  +++
    //
    //  Клонує повністю даний елемент і вертає новий інстанс
    clone: function(opt) {
    ...
    //  +++
    //  
    //  Можна сетати кастомні властивості. Також викликається із метода attr
    //
    ...
    prop: function(props, value, opt) {
        ...
        //  сетить в внутрішні властивості моделі дефолтні поля і поле attr із переданими атрибутами
    ...
    //  +++
    //
    //  Зручний спосіб присвоєння вкладених SVG атрибутів 
    ...
    //  +++
    //  
    //  Дозволяє робити анімацію для елемента
...
//
//
//  Базова вюха і контролер для joint.dia.ElementView і joint.dia.LinkView
...
joint.dia.CellView = Backbone.View.extend({
    ...
    //  Походу цей метод призначений для генерування івентів для paper
    //  
    notify: function(evt) {
...
//  +++
//
//  Добавляє лише додаткові методи і дефолтні атрибути
//
//  Базова модель елемента, властивості якої можна обєднати в наступні групи: Geometry, Presentation, Nesting
//      Geometry
//          - position  або через метод translate
//          - angle     (кут повороту)
//          - size      або доступ через метод resize
//      
//      Presentation оприділяє розмітку для вюхи елемента, яка потім буде відрендерена
//          - attrs
//      
//      Nesting
//          - embeds список елементів, які вбудовані в елемент
//          - parent id батьківського елемента
//  
...
joint.dia.Element = joint.dia.Cell.extend({
    ...
    //  +++ Перевіряв transition для елемента.
    //
    //  Дозволяє робити анімацію для елементів. Викликаючи метод transition
    ...
    //  +++
    //  
    //  Відрізняється від scale 
    //  Змінює розмір елемента. Ліва верхня точка залишається на місці, міняється тільки висота і ширина елемента
...
//  
//  
// joint.dia.ElementView base view and controller.
// -------------------------------------------
...
joint.dia.ElementView = joint.dia.CellView.extend({
    ...
    //  Застосовує атрибути збережені в attrs до SVG розмітки
    //
    ...
    //  +++
    //  
    //  Витягує розмітку з моделі і вставляє її в вюху
    //  
    ...
    render: function() {
        ...
        //  видалити всі дочірні елементи
        this.$el.empty();

        //  Вставляє в вюху SVG розмітку із моделі
        this.renderMarkup();

        //  Проходиться по всіх атрибутах призначених для SVG, сетить їх... відмальовує сам елемент
        this.update();
    ...
    pointerdown: function(evt, x, y) {
         ...
         * 09.07.2015
         * TODO 
         * createLink       кастомний атрибут в HTML розмітці. Оприділяє поведінку опрацювання івента 
         */
        if ( evt.target.getAttribute('createLink') ) {
...
//              
//      Presentation
//          - vertices          масив точок згину лінка
//          - connector 
//          - router            бере масив із vertices і трансформує в масив точок 
//                                  різниця, в тому що vertices визначаються юзером, а route обчислюється
//                                  
//          Routers поділяються:
//              - manhattan         smartRouters автоматично обминають елементи, які лежать на їхньому маршруті
//                                      Генерує точки, які можуть бути зєднанні тільки перпендикулярними лініями
//              - metro             smartRouters можна зєднювати також діагональними лініями
//              - orthogonal        Генерує точки, які можуть бути зєднанні тільки перпендикулярними лініями
//              
//          сonnector бере точки згенеровані роутером і будує SVG path. 
//              - normal            прямі лінії
//              - smooth            те саме що normal, тільки в кінці розгладжує лінії
//              - rounded           використовує криву Безє
//                              
//          
//  
...
joint.dia.LinkView = joint.dia.CellView.extend({
    ...
    //  +++
    //  викликається з методу initialize
    startListening: function() {
        ...
        //  Спрацьовує коли змінюється Target для лінка
    ...
    //  можна дописувати свої додаткові кнопки над лінком. Наприклад кнопка видалення
    ...
    //  +++
    //
    //  приватна функція, для приготування аргументів для функції validateConnection
    ...
    startArrowheadMove: function(end) {
        ...
        //console.log("startArrowheadMove " + end);
    ...
    _beforeArrowheadMove: function() {
        ...
        //  console.log("_beforeArrowheadMove trigger(batch:start)");
    ...
    //  Викликається з paper, якщо .findView(evt.target) знаходить вюху
    /**
     * [pointerdown description]
     * @param  {[object]} evt [jQuery event]
     * @param  {[type]} x y  [координати відносно paper. Можна заюзати функцію snapToGrid]
     */
    pointerdown: function(evt, x, y) {
    ...
    pointermove: function(evt, x, y) {
        ...
        switch (this._action) {
         ...
         /**
          *  Якщо для paper вказаний snapLinks, тоді в певній площі шукаються всі вюхи.
          *  Далі для внутрішньої властивості this._closestView видаляється рамка, які підсвічує виділений елемент,
          *     this._closestView зануляється
          *  Потім проходимся циклом по всіх вюхав в даній площі
          */
          case 'arrowhead-move':

            if ( this.paper.options.snapLinks ) {
                //  знайти всі вюхи в певній області
                ...
                _.each(viewsInArea, function(view) {
                    ...
                    if (view.el.getAttribute('magnet') !== 'false') {
                        ...
                        //  ТУТ прокол. дистанція получається надто великою, тому елемент не рендериться
                        //  the connection is looked up in a circle area by `distance < r`
                        console.log(distance);
                        if (distance < r && distance < minDistance) {

                            //  Тут викликається функція для валідації зєднання
                ...
                //  ЗДАЄТЬСЯ ЦЕЙ МЕТОД ДОМАЛЬОВУЄ РАМКУ НАВКОЛО ЕЛЕМЕНТА
                this._closestView && this._closestView.highlight(this._closestEnd.selector, { connecting: true, snapping: true });

                //  засетати в модель лінка властивість target - id елемента, або координати курсора
...
//  !!! Розібратися, важлива частина
//  
//  options passed to constructor
//      - el                    селектор, jQuery або DOM елемент, який буде контейнером для SVG
//      - width, height
//      - origin                ??? Не бачу різниці
//      - gridSize              розмір рядочків і стовпців сітки, в пікселях
//      - model                 модель графа
//      - perpendicularLinks    ??? Не бачу різниці
//      - elementView           default:ElementView обєкт відповідальний за відмалювання моделей елементів на paper
//      - linkView              default:LinkView    обєкт відповідальний за відмалювання моделей лінків на paper 
//      - defaultLink           default:joint.dia.Link.defaultLink обєкт типу Link, який повинен бути створений, коли юзер наприклад створює лінк із порта 
//                                  може бути функція, яка повертає обєкт типу Link
//      - interactive           взяємодія із елементами. Можна передати функцію. Для link можна заборонити дефолтну поведінку і реалізувати свою
//      - validateMagnet        вирішує чи потрібно створити лінк при кліку на magnet(port). 
//      - validateConnection    вирішує чи дозволити зєднання для лінка між його елементами
//      - linkConnectionPoint   функція, яка дозволяє визначити, яка точка лінка повинна бути приклеїна. Повертає 
//                                  точку з координатами. shapePerimeterConnectionPoint
//      - snapLinks             радіус в якому лінк самостійно починає зєднуватися з портами, або елементами    
//      - markAvailable         Коли починаєш перетягувати лінк, позначити всі доступні порти з класом .available-magnet
//      - async                 не блокує UI, коли потрібно відмалювати багато елементів
//      - embeddingMode         Якщо елементи перетягнути один на одного, верхній елемент вбудовується в нижній
//      - validateEmbedding     функція для перевірки, які елементи можна вбудовувати, а які ні
//      
//  Вюха для графа
//  
//      JointJS library.
...
joint.dia.Paper = Backbone.View.extend({
    ...
    //  +++
    //  
    //  Створює і вертає вюху для моделі,
    createViewForModel: function(cell) {
        ...
        //  Якщо є спеціальна вюха для цієї моделі, тоді використати її замість дефолтних
        ...
        /**
         *  Custom code 04.08.2015
         */
        /*else if ( module === "wtel" && entity === "IF_START" ) {
    ...
    //  +++
    //
    //  Опрацьовує івент додавання нового елемента до графа. Елемент вже доданий в модель графа
    ...
    //  +++
    //  
    //  Створює вюху, рендерить її
    addCell: function(cell) {


        var view = this.createViewForModel(cell);

        //  V ліба для роботи із SVG
        //  view.el стрічка SVG елемента
    ...
    //  +++
    //  
    //  Конвертує глобальні координати, до координат відповідно paper
    ...
    getDefaultLink: function(cellView, magnet) {
        ...
        if ( _.isFunction(this.options.defaultLink) ) {
            response = this.options.defaultLink.call(this, cellView, magnet);
        } else {
            response = this.options.defaultLink.clone();
        }
...
/**
 *  PortsModelInterface це спільний інтерфейс для фігур, які мають порти. Він дозволяє легко створити
 *  нову фігуру з функціональністю портів. Він припускає, що нова фігура має inPorts, outPorts властивості.
 *  Тільки ці властивості потрібно використовувати для призначення портів.
 *  Іншими словами, використання цього інтерфейсу більше не рекомендує сетати порти безпосередньо через
 *  attrs обєкт
 */
